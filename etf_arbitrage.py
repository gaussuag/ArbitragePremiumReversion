"""
ETF套利分析 - MVP版本
使用 adata 获取ETF价格数据，AKShare获取净值数据
包含跟踪误差分析功能
"""
...
class GlobalIndexFetcher:
    """全球指数数据获取器（支持配置化数据源，带防封禁策略）"""

    def __init__(
        self, benchmark_name=None, cache_dir="data/cache", min_delay=1.0, max_delay=2.0
    ):
        """
        初始化
    ...
    def get_index_data(self, start_date=None, end_date=None):
        ...
        if cached is not None and len(cached) > 0:
            ...
            if query_start >= cache_start and query_end <= cache_end:
                filtered = cached[
                    (cached["日期"] >= query_start) & (cached["日期"] <= query_end)
                ]
            ...
            if need_fetch:
                print(
                    f"  本地缓存: {cache_start.strftime('%Y-%m-%d')}至{cache_end.strftime('%Y-%m-%d')}, 需补充: {query_start.strftime('%Y-%m-%d')}至{query_end.strftime('%Y-%m-%d')}",
                    end=" ",
                )
...
class ETFDataFetcher:
    ...
    def __init__(self, cache_dir="data/cache", min_delay=1.0, max_delay=2.0):
        self.cache = ETFCacheManager(cache_dir)
        self.min_delay = min_delay
        self.max_delay = max_delay
    ...
    def get_etf_data(
        self, etf_code, start_date=None, end_date=None, expected_days=None
    ):
        """
        获取ETF的完整数据（价格+净值）
        ...
        print(
            f"  合并后: {len(merged)}条数据 ({merged['日期'].min().strftime('%Y-%m-%d')} 至 {merged['日期'].max().strftime('%Y-%m-%d')})"
        )
    ...
    def _get_price_data(self, etf_code, start_date=None, end_date=None):
        """
        获取ETF历史价格数据（使用 adata 接口，支持增量更新）
        """
        ...
        if cached is not None and len(cached) > 0:
            ...
            if query_start >= cache_start and query_end <= cache_end:
                ...
                print(
                    f"使用缓存 ({len(filtered)}条，缓存范围: {cache_start.strftime('%Y-%m-%d')}至{cache_end.strftime('%Y-%m-%d')})",
                    end=" ",
                )
                return filtered
        ...
        # 从 adata 接口获取 ETF 价格数据
        try:
            # adata 暂时不需要延迟策略（多数据源融合，可用性更高）
            # self._random_sleep()

            # 使用 adata 获取 ETF 日K线数据
            # k_type=1 表示日K线，参数名是 fund_code 不是 stock_code
            ...
            if df is not None and len(df) > 0:
                # 转换列名（adata 返回的列名映射为中文）
                # adata 返回列名: trade_date, open, close, high, low, volume, amount, change_pct
                column_mapping = {
                    "trade_date": "日期",
                    "open": "开盘",
                    "close": "收盘",
                ...
                # 按日期过滤（adata 可能返回 start_date 之后的所有数据）
                df = df[(df["日期"] >= fetch_start) & (df["日期"] <= fetch_end)]
                ...
                # 添加 akshare 原有但 adata 可能没有的标准列
                extra_cols = ["振幅", "涨跌幅", "涨跌额", "换手率"]
        ...
        for attempt in range(3):
            try:
                ...
                df = ak.fund_open_fund_info_em(
                    symbol=etf_code, indicator="单位净值走势", period="成立来"
                )
...
def analyze_arbitrage_opportunities(data_dict, threshold=2.0):
    ...
    for i in range(len(etf_codes)):
        for j in range(i + 1, len(etf_codes)):
            ...
            print(
                f"  套利机会: {opportunities}次 ({opportunities / len(merged) * 100:.1f}%)"
            )
...
def print_summary(data_dict, total_opportunities, all_results):
    ...
    for code, df in data_dict.items():
        ...
        std = df["溢价率"].std()
        print(
            f"  {code}: 平均{avg:+.2f}% | 最高{max_p:+.2f}% | 最低{min_p:+.2f}% | 波动{std:.2f}%"
        )

    # 套利机会汇总
    if all_results:
        max_spread = max(
            [float(r["最大绝对差值"].replace("%", "")) for r in all_results]
        )
        best_pair = max(
            all_results, key=lambda x: float(x["最大绝对差值"].replace("%", ""))
        )

        print(f"\n套利机会汇总:")
        print(f"  总套利机会: {total_opportunities}次")
        print(
            f"  最佳套利对: {best_pair['ETF对']} (最大差值 {best_pair['最大绝对差值']})"
        )
...
def main():
    ...
    parser.add_argument(
        "--threshold", "-t", type=float, help="套利阈值 (%%)", default=2.0
    )
    ...
    if config:
        ...
        THRESHOLD = (
            args.threshold if args.threshold != 2.0 else config.get("threshold", 2.0)
        )
    ...
    if index_df is not None and len(index_df) > 0:
        print(
            f"  {benchmark}数据: {len(index_df)}条 ({index_df['日期'].min().strftime('%Y-%m-%d')} 至 {index_df['日期'].max().strftime('%Y-%m-%d')})"
        )
        ...
        for code, df in data_dict.items():
            ...
            if te_df is not None and len(te_df) > 0:
                ...
                print(
                    f"  {code}: {len(te_df)}条数据 | 平均跟踪误差: {te_df['跟踪误差'].abs().mean():.2f}% | 跟踪精度: {r2 * 100:.2f}%"
                    if r2
                    else f"  {code}: {len(te_df)}条数据"
                )
    ...
    # 分析套利机会
    total_opportunities, all_results = analyze_arbitrage_opportunities(
        data_dict, THRESHOLD
    )
